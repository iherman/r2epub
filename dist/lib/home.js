...
<head>
    <meta charset='utf-8'>
    <title>Service to convert W3C Technical Reports to EPUB 3.3</title>
    <link rel='stylesheet' href='https://iherman.github.io/r2epub/assets/css/convert.css' />
</head>

<body>
    <main>
        <h1>Service to convert W3C Technical Reports to EPUB 3.3</h1>

        <p>This is an interface an on-line service to convert W3C Technical Reports, authored in <a href="https://github.com/w3c/respec/wiki/">ReSpec</a>, to EPUB 3.3</p>

        <p>
            The service receives the parameters through the form above, and generates an EPUB 3.3 instance stored on the local machine. The possible form parameters are:
        </p>
        ...
        <p>
            Convert the HTML file (as generated by ReSpec) to an EPUB 3.3 file. The generated publication's name is <code>short-name.epub</code>, where <code>short-name</code> is set in the ReSpec configuration:
        </p>

        <pre>%%%SERVER%%%?url=https://www.example.org/doc.html</pre>

        <p>Convert the HTML <em>ReSpec source</em> to an EPUB 3.3 file. The source is converted on-the-fly by respec:</p>

        <pre>%%%SERVER%%%?url=https://www.example.org/doc.html&respec=true</pre>

        <p>Convert the HTML <em>ReSpec source</em> to an EPUB 3.3 file, setting its spec status to REC. The source is converted on-the-fly by respec, overwriting the <code>specStatus</code> entry in the configuration to <code>REC</code>:</p>

        <pre>%%%SERVER%%%?url=https://www.example.org/doc.html&respec=true&specStatus=REC</pre>

        <p>Generates a collection of HTML documents to an EPUB 3.3 file, described by a <a href='https://iherman.github.io/r2epub/typedoc/modules/_clib_args_.html'>(JSON) configuration file</a>:</p>
